  ...
  def get_gpt4_response(self, query):
      try:
          ...
          prompt = str("using the commands move() and turn() to control a robot to draw a visual representation of a given user input." +
                       "move() moves in a straight line and turn() takes in a radian input which rotates the direction the robot is facing, which must be a numeric decimal.\n" +
                       "Only return python code with lines containing the methods pupper.move() and pupper.turn(). No english text.\n" +
  ...
  def execute_robot_command(self, response):
      ...
      for line in response.split("\n"):
          ...
          if "pupper.move()" in line:
              self.pupper.move()
          elif "pupper.turn(" in line:
              radians = float(line[line.find("(") + 1: line.find(")")])
              self.pupper.turn(radians)
